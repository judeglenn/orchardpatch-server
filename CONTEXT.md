...
Last updated: June 23, 2026 (Phase 6 implementation complete; Jude's machine agent deploy pending)
...
- GITHUB_TOKEN -- passed as positional arg to Installomator version checks
 (process.env.GITHUB_TOKEN). Lifts GitHub API rate limit from 60/hr to
 5000/hr for GitHub-API-backed labels. Added in Phase 6 agent deploy.
 Add to plist EnvironmentVariables on both machines.
...
- Agent loop split (Phase 6, shipped June 23):
...
## Phase 6 force check-in -- SHIPPED (June 23, 2026)
Design settled in Architectural Deep Dives (Opus). Implementation complete
in the daily Sonnet chat June 23. Jude's machine agent deploy still pending
(manual step -- see open items).
...
Agent (one commit, "Phase 6: fast loop, command poll, GITHUB_TOKEN"):
...
- GITHUB_TOKEN positional arg added to version-checker.js.
- version-checker rewrite (741add4/e071940) now live on Chip's machine.
...
### Verified end to end (Chip's machine, device-C02D52QTML85)
...
- Undo cancel during window: cancel Case A path (no pending_patches row),
 patch_jobs flipped to cancelled, setTimeout guard skips the deferred insert.
- Window expiry: pending_patches row present after 17s, unclaimed, ready for
 agent.

### Still pending (Phase 6 not fully done until this ships)
- Agent deploy to Jude's machine (device-GJM7N0XGL0). Manual step -- Chip
 cannot SSH to Jude's machine. Commands:
 sudo cp -r ~/Projects/orchardpatch-agent/* /usr/local/orchardpatch/agent/
 sudo launchctl bootout system /Library/LaunchDaemons/com.orchardpatch.agent.plist
 sudo launchctl bootstrap system /Library/LaunchDaemons/com.orchardpatch.agent.plist
 tail -n 50 /var/log/orchardpatch/agent.log
 Look for "slow loop: 15min, fast loop: 60s" to confirm new code is live.
 Also add GITHUB_TOKEN to the plist EnvironmentVariables on Jude's machine
 and do a full bootout/bootstrap to pick it up (kickstart -k does not re-read
 plist env vars).
- Visual verification of Undo countdown UI: Jude to queue a silent patch and
 watch the amber countdown in the browser. Chip confirmed the state mechanics
 but cannot operate a browser.
...
- STATUS: 33/45 labels populated in latest_versions. Confirmed working on
 ...
 still runs the prior version-checker -- redeploy pending via the Phase 6
 agent deploy (manual step above).
- GITHUB_TOKEN: now passed as positional arg in Phase 6 agent build. Lifts
 GitHub API rate limit from 60/hr to 5000/hr for GitHub-API-backed labels.
 Needs to be added to plist on both machines.
...
- Chip's machine (device-C02D52QTML85): Phase 6 agent live. Fast loop
 confirmed running. version-checker rewrite live. GITHUB_TOKEN arg added.
- Jude's machine (device-GJM7N0XGL0): still running pre-Phase 6 agent.
 Manual deploy pending -- see Phase 6 section and open items.
...
- latest_versions: label (PK), latest_version, last_checked, error
 ...
 claudedesktop, darkroom, dropboxenterprise, trello.
 - False match / guard-rejected to null: boxdrive, chromeremotedesktop,
 microsoftteams, microsoftteams-rollingout, nomad, coconutbattery.
...
- pending_commands: NEW in Phase 6 (shipped June 23).
...
- **Phase 6: Force check-in.** Fast loop (60s) + slow loop (15min) agent
 ...
 for silent patches. Verified end to end on Chip's machine.
 REMAINING: Jude's machine agent deploy (manual) + amber UI visual check.
...
- Version-resolver redesign: server-side, source-pluggable resolver. Problem
 statement written. HOLD until Phase 6 fully verified (Jude's machine deploy).
 Second Architectural Deep Dive (Opus).

### Not yet built (priority order)
1. Jude's machine Phase 6 agent deploy (complete Phase 6).
2. Version-resolver redesign -- Opus Deep Dive. AFTER Phase 6 verified.
3. Phase 7: Force reinstall in catalog modal (UNINSTALL=1). Schema change
 (force_reinstall BOOLEAN on pending_patches), POST /patch body change,
 agent reads flag and passes UNINSTALL=1 positional arg.
4. MAS app detection: _MASReceipt at inventory time -> source='mas'.
5. Installomator version + Update button on device detail.
6. Console UI redesign -- ELEVATED to outreach-gate. Decide sequencing in
...
- Jude's machine agent deploy: Phase 6 agent + GITHUB_TOKEN plist entry.
 Manual file copy. Tech debt: agent self-update path not built.
- Amber Undo countdown UI: visual check needed in browser (Jude).
- GITHUB_TOKEN: needs to be in plist on both machines. Renewed May 12, 2026,
 scoped to all public repos -- tighten to Installomator repo at next rotation.
...
- 24h unclaimed-expiry cron: fixed to route through terminate_stuck_job in
 Phase 6. Both sweeps now use shared recovery path.
...
- Telegram label mismatch: see Known label-matching issues.
...
- Telegram: "Telegram (Mac App Store)" and "Telegram Desktop" both mapped
 to label 'telegram'. Different versioning schemes. Not investigated.
...
1. Jude's machine Phase 6 agent deploy (manual -- do first, before anything else).
 Then visual check the amber Undo countdown in the browser.
2. Version-resolver redesign Deep Dive (Opus, Architectural Deep Dives channel).
 HOLD until Phase 6 fully verified on both machines.
3. Phase 7: Force reinstall.
4. MAS app detection.
5. Installomator version + Update button on device detail.
6. Console UI redesign sequencing Deep Dive (Opus).
...
- When Chip flags a correctness issue, fix it before committing.
...
- Deferred enqueue (setTimeout) and the cancel handler interact. The guard
 check (SELECT status before INSERT) in the setTimeout body is what makes the
 undo window actually work. Ship both in the same commit.
